...
import { HematologyScene } from './HematologyScene';
import {
  DiagnosticChecklist,
  GenomicGrid,
  PrognosisScoreboard,
...
import {
  ...
  Stethoscope, Droplets, ArrowLeft
} from 'lucide-react';
...
export const CMMLReview: React.FC = () => {
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-3' : 'bg-transparent py-6'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-6">
            <Link to="/" className="flex items-center gap-2 text-slate-500 hover:text-red-700 transition-colors text-xs font-bold uppercase tracking-widest">
              <ArrowLeft size={16} />
              <span className="hidden sm:inline">Blood Doctor</span>
            </Link>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-24 px-6 lg:hidden">
          <div className="flex flex-col gap-6 text-lg font-bold">
            <a href="#pathophysiology" onClick={scrollToSection('pathophysiology')} className="text-slate-600 hover:text-med-red">Pathogenesis</a>
            <a href="#diagnosis" onClick={scrollToSection('diagnosis')} className="text-slate-600 hover:text-med-red">Diagnosis</a>
            <a href="#prognosis" onClick={scrollToSection('prognosis')} className="text-slate-600 hover:text-med-red">Prognosis</a>
            <a href="#therapy" onClick={scrollToSection('therapy')} className="text-slate-600 hover:text-med-red">Therapy</a>
            <a href="#future" onClick={scrollToSection('future')} className="text-slate-600 hover:text-med-red">Future</a>
          </div>
        </div>
      )}
           ...
           <div className="container mx-auto px-6">
              <div className="max-w-4xl mx-auto text-center mb-16">
                 ...
                 <div className="flex flex-wrap justify-center gap-4">
                    ...
                    <div className="px-6 py-2 bg-white border border-slate-200 rounded-full text-xs font-bold text-slate-500 uppercase tracking-widest">HMA &plusmn; BCL2-i</div>
                 </div>
              </div>
        ...
        <section id="authors" className="py-24 bg-slate-100 border-t border-slate-200">
          <div className="container mx-auto px-6 text-center">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                <AuthorBadge name="K. Nachtkamp" affiliation="HHU D&uuml;sseldorf" />
                <AuthorBadge name="F. Schulz" affiliation="HHU D&uuml;sseldorf" />
                <AuthorBadge name="N. Gattermann" affiliation="HHU D&uuml;sseldorf" />
                <AuthorBadge name="U. Germing" affiliation="HHU D&uuml;sseldorf" />
             </div>

             <div className="max-w-2xl mx-auto p-8 border border-slate-200 rounded-lg bg-white shadow-sm">
                <p className="text-slate-500 text-sm italic leading-relaxed">
                  Published in the British Journal of Haematology, 2025. Volume 207, pages 350&ndash;364. This review summarizes the state of the art in clinical management of Chronic Myelomonocytic Leukaemia.
                </p>
      ...
      <footer className="bg-slate-950 text-slate-500 py-16">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex gap-8 text-[10px] font-bold tracking-widest uppercase">
              <Link to="/" className="hover:text-white transition-colors">Blood Doctor Home</Link>
        ...
        <div className="text-center mt-12 text-[9px] text-slate-700 uppercase tracking-[0.3em]">
            &copy; 2025 Blood Doctor &middot; British Journal of Haematology. For medical educational use only.
        </div>
      </footer>
    </div>
  );
